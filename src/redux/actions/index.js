...
export const getAllMovies = () => { 
  return dispatch => {
    return axios.get('http://localhost:3001/movies').then(res => dispatch({ type: GET_ALL_MOVIES, payload: res.data }))
  }
};

export const getMovieDetail = (id) => { 
  return dispatch => {
    return axios.get(`http://localhost:3001/movies/${id}`).then(res => dispatch({ type: GET_MOVIE_DETAILS, payload: res.data }))
  }
};
